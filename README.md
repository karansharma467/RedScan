# 🔎 RedScan

### Authorized Reconnaissance & Vulnerability Assessment Framework

RedScan is a Python-based cybersecurity reconnaissance tool designed for
authorized security testing and learning.

It performs TCP port scanning, service identification, HTTP inspection,
reverse DNS reconnaissance, TCP banner detection, basic HTTP security
checks, risk analysis, and multi-format report generation.

---

## ⚠️ Legal Notice

RedScan should only be used against systems that you own or have explicit
permission to test.

Do not scan public systems, networks, or websites without authorization.

---

## 🚀 Features

- 🎯 IP address and hostname validation
- 📡 Target reachability checking
- 🔍 TCP port scanning
- 🛠️ Service identification
- ⏱️ Configurable connection timeout
- 🔀 Multiple scan modes
- 🏷️ TCP banner detection
- 🌐 HTTP service inspection
- 🔎 Reverse DNS / hostname lookup
- 🛡️ Basic HTTP security-header checks
- 📊 Risk/severity summary
- 📄 TXT scan reports
- 🗂️ JSON scan reports
- 🌐 HTML scan reports
- 🧪 Automated tests using pytest
- ❌ Friendly input-error handling

---

## 🔀 Scan Modes

### Quick

Scans common ports:

```text
22, 80, 443, 8080
```
## Demo

![RedScan Scan Demo](docs/redscan-demo.png)
